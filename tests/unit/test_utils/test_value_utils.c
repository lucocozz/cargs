...
Test(value_utils, compare_values)
{
    ...
    cargs_value_t a = {.as_int = 42};
    cargs_value_t b = {.as_int = 42};
    cargs_value_t c = {.as_int = 100};
    ...
    cargs_value_t str1 = {.as_string = "abc"};
    cargs_value_t str2 = {.as_string = "abc"};
    cargs_value_t str3 = {.as_string = "def"};
    ...
    cargs_value_t true_val = {.as_bool = true};
    cargs_value_t false_val = {.as_bool = false};
    ...
    cargs_value_t float1 = {.as_float = 3.14};
    cargs_value_t float2 = {.as_float = 3.14};
    cargs_value_t float3 = {.as_float = 2.71};
    ...
    cargs_value_t empty_str = {.as_string = ""};
    cargs_value_t null_str = {.as_string = NULL};
...
Test(value_utils, choices_to_value)
{
    ...
    cargs_value_t int_choices = {.as_array_int = (long long[]){10, 20, 30, 40, 50}};
    
    cargs_value_t result1 = choices_to_value(VALUE_TYPE_INT, int_choices, 5, 2);
    cr_assert_eq(result1.as_int, 30, "Third element should be 30");
    
    cargs_value_t result2 = choices_to_value(VALUE_TYPE_INT, int_choices, 5, 0);
    cr_assert_eq(result2.as_int, 10, "First element should be 10");
    
    // Test with invalid index
    cargs_value_t invalid = choices_to_value(VALUE_TYPE_INT, int_choices, 5, 10);
    cr_assert_eq(invalid.raw, 0, "Invalid index should return empty value");
    
    // Test string choices
    cargs_value_t str_choices = {.as_array_string = (char*[]){"one", "two", "three"}};
    
    cargs_value_t str_result = choices_to_value(VALUE_TYPE_STRING, str_choices, 3, 1);
    cr_assert_str_eq(str_result.as_string, "two", "Second element should be 'two'");
    
    // Test float choices
    cargs_value_t float_choices = {.as_array_float = (double[]){1.1, 2.2, 3.3}};
    
    cargs_value_t float_result = choices_to_value(VALUE_TYPE_FLOAT, float_choices, 3, 2);
    cr_assert_float_eq(float_result.as_float, 3.3, 0.0001, "Third element should be 3.3");
    
    // Test with negative index
    cargs_value_t neg_index = choices_to_value(VALUE_TYPE_INT, int_choices, 5, -1);
