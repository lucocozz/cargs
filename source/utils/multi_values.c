...
static int _compare_int_values(const void *a, const void *b)
{
	...
	if (va->as_int < vb->as_int) return -1;
	if (va->as_int > vb->as_int) return 1;
	return 0;
}
...
static int _compare_string_values(const void *a, const void *b)
{
	...
	if (!va->as_string) return -1;
	if (!vb->as_string) return 1;
...
static int _compare_float_values(const void *a, const void *b)
{
	...
	if (va->as_float < vb->as_float) return -1;
	if (va->as_float > vb->as_float) return 1;
	return 0;
}
...
static int _compare_map_int_values(const void *a, const void *b)
{
	...
	if (pa->value.as_int < pb->value.as_int) return -1;
	if (pa->value.as_int > pb->value.as_int) return 1;
	return 0;
}
...
static int _compare_map_string_values(const void *a, const void *b)
{
	...
	if (!pa->value.as_string) return -1;
	if (!pb->value.as_string) return 1;
...
static int _compare_map_float_values(const void *a, const void *b)
{
	...
	if (pa->value.as_float < pb->value.as_float) return -1;
	if (pa->value.as_float > pb->value.as_float) return 1;
	return 0;
}
			...
			switch (type) {
				...
				case VALUE_TYPE_MAP_STRING:
					if (map[i].value.as_string && map[j].value.as_string)
						is_duplicate = (strcmp(map[i].value.as_string, map[j].value.as_string) == 0);
					break;
					
				case VALUE_TYPE_FLOAT:
				case VALUE_TYPE_MAP_FLOAT:
					is_duplicate = (fabs(map[i].value.as_float - map[j].value.as_float) < 0.0000001);
					break;
...
void apply_map_flags(cargs_option_t *option)
{
	...
	{
		option->value_count = make_map_values_unique(
			option->value.as_map, 
			option->value_count, 
			option->value_type
		);
...
void	adjust_array_size(cargs_option_t *option)
{
	if (option->value.as_array == NULL) {
		...
		option->value.as_array = realloc(option->value.as_array, option->value_capacity * sizeof(value_t *));
	}
}
...
void	adjust_map_size(cargs_option_t *option)
{
	if (option->value.as_map == NULL) {
		...
		option->value_capacity *= 2;
		option->value.as_map = realloc(option->value.as_map, option->value_capacity * sizeof(value_t *));
	}
}
